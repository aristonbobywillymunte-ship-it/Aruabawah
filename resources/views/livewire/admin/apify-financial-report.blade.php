<div class="w-full space-y-6">
                        ...
                        <tr class="hover:bg-slate-50/50 transition">
                            ...
                            <td class="px-4 py-3 text-slate-500 text-center">
                                @if($run['items'] > 0)
                                    ...
                                    >
                                        <span wire:loading.remove wire:target="openItems({{ $run['id'] }})" class="material-symbols-outlined text-[13px] font-bold">visibility</span>
                                        <svg wire:loading wire:target="openItems({{ $run['id'] }})" class="animate-spin h-3 w-3 text-[#1fa387]" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
    ...
    <div wire:key="apify-financial-items-modal"
         ...
         x-show="open"
         x-cloak
         @keydown.escape.window="if (open) $wire.closeItemsModal()"
        ...
        <div class="w-full max-w-6xl bg-white shadow-2xl text-left flex flex-col rounded-[24px] overflow-hidden max-h-[92vh]">
            ...
            <div class="flex-1 overflow-y-auto p-4 sm:p-6 relative min-h-[300px] overscroll-contain">
                ...
                {{-- Loading State --}}
                @if($modalLoading)
                    <div class="absolute inset-0 flex flex-col items-center justify-center bg-white/80 z-10 space-y-2">
                        <span class="material-symbols-outlined text-[32px] text-[#1fa387] animate-spin">progress_activity</span>
                        <span class="text-xs font-bold text-slate-600">Mengambil data hasil scraping...</span>
                    </div>
                @endif

...
<div class="mx-auto w-full max-w-7xl space-y-6 font-sans">
    ...
    <div wire:loading.flex wire:target="saveActor, requestToggleActorStatus, toggleActorStatusConfirmed, requestDeleteActor, deleteActorConfirmed, testConnection, editActor, saveToken" class="fixed inset-0 z-50 items-center justify-center bg-slate-950/20 backdrop-blur-[2px]">
        <div class="bg-white px-5 py-3 rounded-2xl shadow-xl flex items-center gap-3 border border-slate-100 animate-fade-in">
            ...
            <span class="text-xs font-bold text-slate-700">Memproses data...</span>
        </div>
    </div>

    <div class="rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-[11px] font-medium leading-relaxed text-amber-900 shadow-sm text-left">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div class="text-left">
                        <div class="flex items-center justify-between mb-1">
                            ...
                            <span class="inline-flex rounded-full px-2 py-0.5 text-[8px] font-black uppercase tracking-wider 
                                {{ $connectionStatus === 'connected' ? 'bg-emerald-50 text-emerald-700 border border-emerald-100' : ($connectionStatus === 'error' ? 'bg-rose-50 text-rose-700 border border-rose-100' : 'bg-slate-100 text-slate-500') }}">
                                {{ $connectionStatus === 'connected' ? 'Connected' : ($connectionStatus === 'error' ? 'Error' : 'Belum Dicek') }}
                            </span>
                        </div>
                        <input 
                    ...
                    <div class="text-left">
                        <div class="flex items-center justify-between mb-1">
                            ...
                            @if(filled($apiTokenBackup1))
                                <span class="inline-flex rounded-full px-2 py-0.5 text-[8px] font-black uppercase tracking-wider 
                                    {{ $connectionStatusBackup1 === 'connected' ? 'bg-emerald-50 text-emerald-700 border border-emerald-100' : ($connectionStatusBackup1 === 'error' ? 'bg-rose-50 text-rose-700 border border-rose-100' : 'bg-slate-100 text-slate-500') }}">
                                    {{ $connectionStatusBackup1 === 'connected' ? 'Connected' : ($connectionStatusBackup1 === 'error' ? 'Error' : 'Belum Dicek') }}
                                </span>
                            @else
                    ...
                    <div class="text-left">
                        <div class="flex items-center justify-between mb-1">
                            ...
                            @if(filled($apiTokenBackup2))
                                <span class="inline-flex rounded-full px-2 py-0.5 text-[8px] font-black uppercase tracking-wider 
                                    {{ $connectionStatusBackup2 === 'connected' ? 'bg-emerald-50 text-emerald-700 border border-emerald-100' : ($connectionStatusBackup2 === 'error' ? 'bg-rose-50 text-rose-700 border border-rose-100' : 'bg-slate-100 text-slate-500') }}">
                                    {{ $connectionStatusBackup2 === 'connected' ? 'Connected' : ($connectionStatusBackup2 === 'error' ? 'Error' : 'Belum Dicek') }}
                                </span>
                            @else
                                <span class="text-[9px] text-slate-350 italic">Kosong</span>
                    ...
                    <div class="text-left">
                        <div class="flex items-center justify-between mb-1">
                            ...
                            @if(filled($apiTokenBackup3))
                                <span class="inline-flex rounded-full px-2 py-0.5 text-[8px] font-black uppercase tracking-wider 
                                    {{ $connectionStatusBackup3 === 'connected' ? 'bg-emerald-50 text-emerald-700 border border-emerald-100' : ($connectionStatusBackup3 === 'error' ? 'bg-rose-50 text-rose-700 border border-rose-100' : 'bg-slate-100 text-slate-500') }}">
                                    {{ $connectionStatusBackup3 === 'connected' ? 'Connected' : ($connectionStatusBackup3 === 'error' ? 'Error' : 'Belum Dicek') }}
                                </span>
                            @else
                                <span class="text-[9px] text-slate-350 italic">Kosong</span>
        ...
        <div class="rounded-3xl border border-slate-200 bg-white shadow-sm text-left flex flex-col overflow-hidden">
            <div class="flex items-center justify-between border-b border-slate-100 px-6 py-4">
                ...
                <button 
                    ...
                    class="inline-flex h-10 w-full sm:w-auto items-center justify-center gap-2 rounded-2xl bg-indigo-600 px-4 text-xs font-bold text-white shadow-sm transition hover:bg-indigo-700 cursor-pointer disabled:opacity-50"
                >
                    <span wire:loading.remove wire:target="syncManagedActors" class="material-symbols-outlined text-[17px]">sync</span>
                    <span wire:loading wire:target="syncManagedActors" class="animate-spin material-symbols-outlined text-[17px]">progress_activity</span>
            ...
            <div class="w-full max-w-4xl overflow-hidden rounded-[24px] bg-white shadow-2xl text-left overscroll-contain flex flex-col max-h-[92vh]">
                <div class="flex items-center justify-between border-b border-slate-100 px-6 py-4">
                    <div class="min-w-0 flex-1 pr-4">
                        ...
                        @if($editingActor)
                            <h2 class="text-base font-black text-slate-900 mt-0.5 truncate">
                                Ubah: {{ $platform }}
                                @if($actorName)
                                    &mdash; {{ $actorName }}
                                @endif
                            ...
                            @if($actorSlug)
                                <p class="text-[10px] font-mono text-slate-400 mt-0.5 truncate">{{ $actorSlug }}</p>
                            @endif
                        @else
                            <h2 class="text-base font-black text-slate-900 mt-0.5">Tambah Aktor Apify Baru</h2>
                        @endif
                    </div>
                    <button type="button" wire:click="closeActorModal" class="rounded-full p-2 text-slate-400 hover:bg-slate-100 hover:text-slate-700 transition cursor-pointer shrink-0">
                                ...
                                <div class="border-t border-slate-100 bg-blue-50/30 p-4 space-y-4 rounded-b-2xl">
                                    ...
                                    <div class="grid gap-4 sm:grid-cols-2">
                                        <div>
                                            ...
                                            <input wire:model="facebook_max_posts" type="number" required class="h-10 w-full rounded-xl border border-slate-200 px-3.5 text-xs font-semibold text-slate-800 outline-none focus:border-[#1fa387] focus:ring-2 focus:ring-[#1fa387]/10 transition bg-white shadow-sm">
    ...
    @if($confirmingDelete)
        <div 
            wire:key="admin-apify-actor-delete-modal-{{ $deleteId ?? 'none' }}"
        ...
        >
            <div class="w-full max-w-sm rounded-[24px] bg-white p-6 shadow-2xl text-left space-y-4 overscroll-contain">
                ...
                <div class="flex items-center justify-end gap-3 pt-2">
                    <button wire:click="$set('confirmingDelete', false)" wire:loading.attr="disabled" wire:target="deleteActorConfirmed" class="h-10 rounded-xl border border-slate-200 px-5 text-xs font-bold text-slate-600 hover:bg-slate-50 transition cursor-pointer disabled:opacity-50">Batal</button>
                    <button 
                    ...
                    >
                        <span wire:loading wire:target="deleteActorConfirmed" class="animate-spin material-symbols-outlined text-[18px]">progress_activity</span>
    ...
    @if($confirmingToggle)
        <div 
            wire:key="admin-apify-actor-toggle-modal-{{ $toggleId ?? 'none' }}"
        ...
        >
            <div class="w-full max-w-sm rounded-[24px] bg-white p-6 shadow-2xl text-left space-y-4 overscroll-contain">
                ...
                <div class="flex items-center justify-end gap-3 pt-2">
                    <button wire:click="cancelToggle" wire:loading.attr="disabled" wire:target="toggleActorStatusConfirmed" class="h-10 rounded-xl border border-slate-200 px-5 text-xs font-bold text-slate-600 hover:bg-slate-50 transition cursor-pointer disabled:opacity-50">Batal</button>
                    @if($toggleCurrentStatus === 'active')
                        <button 
                        ...
                        >
                            <span wire:loading wire:target="toggleActorStatusConfirmed" class="animate-spin material-symbols-outlined text-[18px]">progress_activity</span>
    ...
    @if($showTestModal)
        ...
        @endphp
        <div 
            wire:key="admin-apify-test-modal-{{ $testingActorId ?? 'none' }}"
            ...
            <div class="w-full max-w-lg overflow-hidden rounded-[24px] bg-white shadow-2xl text-left overscroll-contain">
                ...
                <form wire:submit.prevent="runTest" class="p-6 space-y-4">
                    ...
                    <div class="flex items-center justify-end gap-3 pt-3 border-t border-slate-100 font-sans">
                        <button type="button" wire:click="closeTestModal" wire:loading.attr="disabled" wire:target="runTest" class="h-10 rounded-xl border border-slate-200 px-5 text-xs font-bold text-slate-600 hover:bg-slate-50 transition cursor-pointer disabled:opacity-50">Batal</button>
                        <button 
                            type="submit" 
                            wire:loading.attr="disabled"
                            wire:target="runTest"
                            class="inline-flex items-center justify-center gap-2 h-10 rounded-xl bg-[#1fa387] hover:bg-[#1a8b73] text-white px-6 text-xs font-bold transition cursor-pointer disabled:opacity-50"
                        >
                            <span wire:loading wire:target="runTest" class="animate-spin material-symbols-outlined text-[18px]">progress_activity</span>

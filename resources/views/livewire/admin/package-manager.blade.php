                ...
                <div class="p-6 md:p-7 flex flex-col flex-1">
                    ...
                    <div class="mb-5">
                        ...
                        <div class="mt-3 grid grid-cols-2 gap-2 bg-slate-50 rounded-xl p-2.5 border border-slate-100 text-[10px] text-slate-500 font-semibold">
                            ...
                            <div class="flex flex-col gap-0.5 mt-1 pt-1 border-t border-slate-200 col-span-2">
                                <span class="material-symbols-outlined text-[14px] text-violet-500">newspaper</span>
                                @if($pkg->news_runs_per_day && !empty($pkg->news_run_times))
                                    <span>
                                        Portal: {{ $pkg->news_runs_per_day }}x / hari
                                        <span class="block text-[9px] font-medium text-slate-400">{{ implode(' · ', $pkg->news_run_times) }}</span>
                                    </span>
                                @else
                                    <span>{{ $this->scheduleLegacySummary('Berita', (int) ($pkg->news_interval_minutes ?? 5)) }}</span>
                                @endif
                            </div>
                            <div class="flex flex-col gap-0.5 mt-1 pt-1 border-t border-slate-200 col-span-2">
                                <span class="material-symbols-outlined text-[14px] text-sky-500">share</span>
                                @if($pkg->social_runs_per_day && !empty($pkg->social_run_times))
                                    <span>
                                        Sosmed: {{ $pkg->social_runs_per_day }}x / hari
                                        <span class="block text-[9px] font-medium text-slate-400">{{ implode(' · ', $pkg->social_run_times) }}</span>
                                    </span>
                                @else
                                    <span>{{ $this->scheduleLegacySummary('Sosmed', (int) ($pkg->social_interval_minutes ?? 10)) }}</span>
                                @endif
                            </div>
                        </div>
                    </div>
            ...
            <div x-init="$el.scrollTop = 0" class="flex-grow overflow-y-auto p-8 space-y-6 text-left" style="flex: 1 1 auto; -webkit-overflow-scrolling: touch; scroll-behavior: smooth; overscroll-behavior: contain; transform: translate3d(0, 0, 0); will-change: scroll-position;">
                ...
                <div class="mb-8">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5 bg-white p-6 rounded-2xl border border-slate-100 shadow-sm">
                        ...
                        <div class="md:col-span-2 rounded-2xl border border-[#1fa387]/10 bg-white/80 p-4 sm:p-5 shadow-[0_2px_15px_-3px_rgba(31,163,135,0.06)] space-y-5">
                            <div class="flex items-center gap-1.5 text-xs font-black text-[#1fa387] uppercase tracking-wider border-b border-[#1fa387]/10 pb-2">
                                <span class="material-symbols-outlined text-[16px] text-[#1fa387]">schedule</span>
                                Jadwal Pengambilan Harian
                            </div>
                ...
                <div class="mb-8">
                    <h3 class="text-sm font-black text-slate-600 mb-4 flex items-center gap-2">
                        <span class="material-symbols-outlined text-[18px] text-[#1fa387]">data_usage</span>
                        Batas Paket
                    </h3>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5 bg-white p-6 rounded-2xl border border-slate-100 shadow-sm">
                        <!-- Max Projects -->
                        <div>
                            <div class="flex items-center justify-between mb-1">
                                <label class="text-[11px] font-bold text-slate-700 tracking-wide">Maksimal Proyek</label>
                                <span class="text-[9px] font-bold text-slate-400 bg-slate-100 px-2 py-0.5 rounded-md">Opsional</span>
                            </div>
                            <p class="text-[10px] text-slate-400 mb-2">Kosongkan untuk tanpa batas.</p>
                            <input wire:model="max_projects" type="number" min="1" placeholder="Contoh: 5"
                                class="w-full px-4 py-2.5 rounded-xl border border-[#1fa387]/20 text-sm focus:outline-none focus:ring-4 focus:ring-[#1fa387]/10 focus:border-[#1fa387] transition-all bg-slate-50/60 hover:bg-white @error('max_projects') border-rose-300 bg-rose-50/50 @enderror" />
                            @error('max_projects') <p class="text-rose-500 text-[11px] font-semibold mt-1">{{ $message }}</p> @enderror
                        </div>

                        <!-- Max Keywords -->
                        <div>
                            <div class="flex items-center justify-between mb-1">
                                <label class="text-[11px] font-bold text-slate-700 tracking-wide">Maksimal Kata Kunci / Proyek</label>
                                <span class="text-[9px] font-bold text-slate-400 bg-slate-100 px-2 py-0.5 rounded-md">Opsional</span>
                            </div>
                            <p class="text-[10px] text-slate-400 mb-2">Kosongkan untuk tanpa batas.</p>
                            <input wire:model="max_keywords_per_project" type="number" min="1" placeholder="Contoh: 25"
                                class="w-full px-4 py-2.5 rounded-xl border border-[#1fa387]/20 text-sm focus:outline-none focus:ring-4 focus:ring-[#1fa387]/10 focus:border-[#1fa387] transition-all bg-slate-50/60 hover:bg-white @error('max_keywords_per_project') border-rose-300 bg-rose-50/50 @enderror" />
                            @error('max_keywords_per_project') <p class="text-rose-500 text-[11px] font-semibold mt-1">{{ $message }}</p> @enderror
                        </div>
                    </div>
                </div>
                        ...
                        @forelse($groupedActors as $platform => $actors)
                            ...
                            <div class="divide-y divide-[#1fa387]/8">
                                @foreach($actors as $actor)
                                    @php
                                        $config = $actorConfig[$actor->id] ?? ['is_enabled' => false, 'cost_per_run_usd' => ''];
                                        $isConfigIncomplete = ($config['is_enabled'] ?? false)
                                            && (
                                                ($config['cost_per_run_usd'] ?? '') === ''
                                                || !is_numeric($config['cost_per_run_usd'] ?? null)
                                                || (float) ($config['cost_per_run_usd'] ?? 0) < 0
                                                || ($config['default_limit'] ?? '') === ''
                                                || !is_numeric($config['default_limit'] ?? null)
                                                || (int) ($config['default_limit'] ?? 0) < 1
                                                || ($config['memory_limit'] ?? '') === ''
                                                || !is_numeric($config['memory_limit'] ?? null)
                                                || (int) ($config['memory_limit'] ?? 0) < 128
                                            );
                                    @endphp
                                    <div wire:key="package-form-actor-{{ $actor->id }}" class="px-5 py-4 hover:bg-slate-50/50 transition-colors flex flex-col lg:flex-row lg:items-center justify-between gap-4 border-b last:border-0 border-slate-100">
                                            ...
                                            <div class="min-w-0">
                                                ...
                                                <div class="flex flex-wrap gap-1.5 mt-2">
                                                    ...
                                                    @if($isConfigIncomplete)
                                                        <span class="inline-flex items-center rounded-md bg-rose-50 px-2 py-0.5 text-[9px] font-bold text-rose-600 border border-rose-100 uppercase tracking-wide">Belum Lengkap</span>
                                                    @endif
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        <div class="flex flex-wrap items-center gap-4 bg-slate-50 border border-slate-100 rounded-2xl p-3 shrink-0 w-full lg:w-auto justify-between lg:justify-end">
                                            {{-- Package-only notice --}}
                                            <div class="text-left px-2 min-w-[120px]">
                                                <div class="text-[8px] text-[#1fa387] font-bold uppercase tracking-wider">Runtime Paket</div>
                                                <div class="text-[10px] font-semibold text-slate-500 mt-0.5 leading-tight">
                                                    Limit, biaya, dan RAM wajib diisi dari paket ini.
                                                </div>
                                            </div>
                                            ...
                                            <div class="text-left px-2">
                                                <div class="text-[8px] text-slate-400 font-bold uppercase tracking-wider mb-1">Biaya Paket ($)</div>
            ...
            <div wire:key="package-actors-actor-{{ $actor->id }}" 
                ...
                <div class="flex items-center gap-4 flex-1 min-w-0">
                    ...
                    <div class="min-w-0">
                        <div class="font-extrabold text-xs text-slate-700 tracking-tight flex flex-wrap items-center gap-2">
                            ...
                            @if($isConfigIncomplete)
                                <span class="inline-flex shrink-0 items-center px-2 py-0.5 rounded-full text-[8.5px] font-black bg-rose-50 text-rose-650 border border-rose-100 uppercase tracking-wider">
                                    Belum Lengkap
                                </span>
                            @endif
                        </div>
                ...
                <div class="flex flex-wrap items-center gap-4 bg-slate-50 border border-slate-100 rounded-2xl p-3 shrink-0 w-full lg:w-auto justify-between lg:justify-end">
                    ...
                    <div class="text-left px-2 min-w-[120px]">
                        <div class="text-[8px] text-[#1fa387] font-bold uppercase tracking-wider">Runtime Paket</div>
                        <div class="text-[10px] font-semibold text-slate-500 mt-0.5 leading-tight">
                            Runtime proyek membaca nilai dari paket ini, bukan actor global.
                        </div>
                    </div>

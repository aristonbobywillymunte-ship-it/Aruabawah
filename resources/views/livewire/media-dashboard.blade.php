...
>
        ...
        <div class="desktop-workspace-area flex-1 min-w-0 flex flex-col lg:flex-row gap-6 px-4 sm:px-8 py-6 items-stretch pb-20 lg:pb-20" wire:key="desktop-workspace-area">
                    ...
                    @else
                        ...
                        >
                            ...
                            <div class="bg-white border border-slate-200 rounded-2xl p-12 text-center space-y-4 shadow-sm">
                                <svg class="w-12 h-12 text-slate-300 mx-auto" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M15.182 16.318A4.486 4.486 0 0012.016 15a4.486 4.486 0 00-3.198 1.318M9 10h.01M15 10h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                                </svg>
                            ...
                            @foreach($articlesList as $article)
                                ...
                                >
                                    ...
                                    @php
                                        ...
                                        $facebookCommentsClickable = false;
                                        if ($isSocial) {
            ...
            @elseif($this->isTab('analisis'))
                ...
                <section class="flex-1 min-w-0 flex flex-col h-full overflow-hidden space-y-4 pr-1" wire:init="loadAnalysis" wire:key="dashboard-analysis-section">
                    ...
                    >
                    @if(!$analysisLoaded)
                        <!-- Comprehensive Skeleton Loader Mirroring All Analisis Cards -->
                        <div class="space-y-6 animate-pulse mt-1">
                            <!-- 1. Gambaran Umum Card Grid Skeleton -->
                            <div class="bg-white rounded-3xl border border-slate-200 p-6 sm:p-8 shadow-sm space-y-6">
                                <div class="pb-3 border-b border-slate-100">
                                    <div class="h-4 w-40 bg-slate-200 rounded"></div>
                                    <div class="h-2.5 w-72 bg-slate-100 rounded mt-2"></div>
                                </div>

                                <!-- Row 1: 3 Big KPI Cards -->
                                <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
                                    @for($i=0; $i<3; $i++)
                                    <div class="border border-slate-200 bg-white rounded-2xl p-5 shadow-sm flex items-center justify-between h-[100px]">
                                        <div class="space-y-2">
                                ...
                                <!-- Row 2: 4 Channel Cards (IG, TikTok, FB, Berita) -->
                                <div class="grid grid-cols-1 sm:grid-cols-2 2xl:grid-cols-4 gap-4">
                                    @for($i=0; $i<4; $i++)
                                    <div class="border border-slate-200 bg-white rounded-2xl p-5 shadow-sm flex flex-col justify-between h-[155px]">
                                        <div class="flex items-center justify-between w-full">
                                            <div class="flex items-center gap-3">
                                                <div class="w-[52px] h-[52px] rounded-2xl bg-slate-100 shrink-0"></div>
                                ...
                                <!-- Row 3: 2 Sentiment Summary Cards (Medsos & Berita) -->
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    @for($i=0; $i<2; $i++)
                                    <div class="border border-slate-200 bg-white rounded-2xl p-5 shadow-sm space-y-3">
                                        <div class="h-4 w-40 bg-slate-200 rounded pb-2 border-b border-slate-100"></div>
                                        <div class="grid grid-cols-3 gap-2">
                                            <div class="h-14 bg-slate-50 border border-slate-100 rounded-xl"></div>
                                            <div class="h-14 bg-slate-50 border border-slate-100 rounded-xl"></div>
                                            <div class="h-14 bg-slate-50 border border-slate-100 rounded-xl"></div>
                                        </div>
                            ...
                            <!-- 2. Grafik Tren Kinerja Proyek Skeleton -->
                            <div class="bg-white rounded-3xl border border-slate-200 p-6 sm:p-8 shadow-sm space-y-4">
                                <div class="flex justify-between items-center pb-2.5 border-b border-slate-100">
                                    <div class="space-y-1.5">
                                        ...
                                        <div class="h-2.5 w-80 bg-slate-100 rounded"></div>
                                    </div>
                                    <div class="h-8 w-56 bg-slate-100 rounded-xl"></div>
                                </div>
                                <div class="h-[200px] bg-slate-50 border border-slate-100 rounded-2xl"></div>
                            </div>

                            <!-- 3. Word Cloud & Category Breakdowns Skeleton -->
                            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                                <div class="bg-white rounded-3xl border border-slate-200 p-6 shadow-sm h-[360px] space-y-4">
                                    ...
                                    <div class="h-[260px] bg-slate-50 rounded-2xl border border-slate-100"></div>
                                </div>
                                <div class="bg-white rounded-3xl border border-slate-200 p-6 shadow-sm h-[360px] space-y-4">
                                    <div class="flex justify-between pb-2 border-b border-slate-100">
                                        <div class="h-4 w-40 bg-slate-200 rounded"></div>
                            ...
                            <!-- 4. Peta Jaringan Isu & Berita Populer Skeleton -->
                            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                                <div class="bg-white rounded-3xl border border-slate-200 p-6 shadow-sm h-[380px] space-y-4">
                                    <div class="h-4 w-48 bg-slate-200 rounded pb-2 border-b border-slate-100"></div>
                                    <div class="h-[280px] bg-slate-50 rounded-2xl border border-slate-100"></div>
                                </div>
                                <div class="bg-white rounded-3xl border border-slate-200 p-6 shadow-sm h-[380px] space-y-4">
                                    <div class="h-4 w-40 bg-slate-200 rounded pb-2 border-b border-slate-100"></div>
                                    <div class="space-y-3">
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="flex justify-end pt-6 border-t border-slate-100">
                                <button
                                    ...
                                    class="bg-[#1fa387] hover:bg-[#178a70] disabled:opacity-60 disabled:cursor-not-allowed text-white font-bold text-xs px-6 py-3 rounded-xl transition flex items-center gap-1.5 cursor-pointer shadow-sm"
                                >
    ...
    <div wire:loading.flex wire:target="preparePdfReport" style="display:none;" class="hidden fixed inset-0 z-[9999] items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4">
        <div class="w-full max-w-md rounded-3xl bg-white shadow-2xl border border-slate-200 p-6 text-center">
            ...
            <h3 class="text-base font-extrabold text-slate-900">Menyusun Laporan PDF</h3>
            <p class="mt-2 text-sm text-slate-500">Sistem sedang merangkum ringkasan dan analisis isu terbaru. Mohon tunggu sebentar.</p>
        </div>
    </div>
